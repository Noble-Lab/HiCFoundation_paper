...
def scan_matrix(matrix, input_row_size,input_col_size, stride,
    ...
    half_row_size = input_row_size//2
    half_col_size = input_col_size//2
    for i in range(0, row_size - input_row_size//2, stride):
        ...
        col_center = i + half_row_size
        col_start = max(0,col_center - input_col_size//2)
        col_end = min(col_size, col_center + input_col_size//2)
        if col_end<=col_center:
            #skip this since the center diagonal is even beyond the region
            continue
        submat_left_index = half_col_size - (col_center - col_start)
        submat_right_index = half_col_size + (col_end - col_center)

        submatrix[:row_end-row_start,submat_left_index:submat_right_index] = matrix[row_start: row_end, col_start: col_end]
        ...
        """
        output_path = os.path.join(output_dir, str(current_chrom) + '_' + str(i) + '_' + str(col_center) + '.pkl')
